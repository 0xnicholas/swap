...
const SwapForm = ({ setPairs }) => {
  ...
  const swap = (e) => {
    ...
    const amountIn = ethers.parseEther(zeroForOne ? amount0 : amount1);
    const amountOut = ethers.parseEther(zeroForOne ? amount1 : amount0);
  ...
  const updateAmountOut = debounce((amount) => {
    ...
    quoter.callStatic
      ...
      .then(({ amountOut }) => {
        zeroForOne ? setAmount1(ethers.formatEther(amountOut)) : setAmount0(ethers.formatEther(amountOut));
        setLoading(false);
      })
      .catch((err) => {
